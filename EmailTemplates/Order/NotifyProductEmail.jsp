...
<wcf:rest var="catentry" url="${searchHostNamePath}${searchContextPath}/store/${WCParam.storeId}/productview/byId/${itemId}" >	
	<wcf:param name="langId" value="${WCParam.langId}"/>
	<wcf:param name="responseFormat" value="json"/>		
	<wcf:param name="catalogId" value="${WCParam.catalogId}"/>
</wcf:rest>
<c:set var="itemName" value="${catentry.catalogEntryView[0].name}" />

<!-- BEGIN ProductNotify.jsp -->
									...
									<tr>
										<td>
											<table style="border-collapse: collapse; border-spacing: 0; margin: 0 auto; width: 558px; color: #404040; font-size: 12px;" role="presentation">
												<tr>
													<td>	
													    <c:set var="emsName" value="ProductNotifyCenter_Content"/>
														...
														<c:param name="substitutionName1" value="[itemId]" />
														<c:param name="substitutionValue1" value="${WCParam.itemId}" />
														<c:param name="substitutionName4" value="[itemName]" />
														<c:param name="substitutionValue4" value="${itemName}" />
														<c:param name="substitutionName2" value="[storeName]" />
														<c:param name="substitutionValue2" value="${storeName}" />
														
														
														<%--
															<c:param name="substitutionName2" value="[requestedItem]" />
															<c:param name="substitutionValue2" value="${WCParam.itemName}" />
