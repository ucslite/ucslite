...
<%-- Harish Starts : Added Loyalty related code snippet v8--%>
...
<wcf:rest var="usablePaymentInfo" url="store/{storeId}/cart/@self/usable_payment_info" scope="request">
		<wcf:var name="storeId" value="${WCParam.storeId}" encode="true"/>
		<wcf:param name="orderId" value="${paymentInstruction.orderId }"/>
	</wcf:rest>
<wcf:rest var="productListOnCart" url="/store/{storeId}/order/{orderId}">
		<wcf:var name="storeId" value="${storeId}" />
		<wcf:var name="orderId" value="${paymentInstruction.orderId }" />
		<wcf:param name="sortOrderItemBy" value="orderItemID"/>
	</wcf:rest>
	
<c:set var="orderTotlaAmount" value="${productListOnCart.grandTotal}" />
<c:set var="remainingBalanceAmount" value="" />
<c:set var="paidLoyaltyAmount" value="" />

<c:forEach var="paymentInst" items="${paymentInstruction.paymentInstruction}" varStatus="paymentInstructionCounter">
	<c:if test="${paymentInst.payMethodId == 'Loyalty'}">
		<c:set var="paymentInstAmount" value="${paymentInst.piAmount}" />
	</c:if>
</c:forEach>

<c:forEach var="paymentInstance" items="${usablePaymentInfo.usablePaymentInformation}" varStatus="paymentCount">
	<c:if test="${paymentInstance.paymentMethodName == 'Loyalty'}">
		<c:set var="remainingBalanceAmount" value="${productListOnCart.grandTotal - paymentInstAmount}" />
		<c:set var="paidLoyaltyAmount" value="${paymentInstAmount}" />
	</c:if>
</c:forEach>

<input type="hidden" value="${paidLoyaltyAmount}" name="loyaltyAmount" id="loyaltyAmountId" />
<c:set var="remainingAmount" value="${remainingBalanceAmount}" scope="request"/>
<c:set var="amount" value=" " />
<c:choose>
	<c:when test="${!empty remainingAmount }">
		<c:set var="orderTotlaAmount" value="${remainingAmount}" />
		<input type="hidden" name="amount" value="<c:out value='${remainingAmount}'/>" id="amountId" />
	</c:when>
	<c:otherwise>
		<input type="hidden" name="amount" value="<c:out value='${productListOnCart.grandTotal}'/>" id="amountId" />
	</c:otherwise>
</c:choose>

<c:set var="piAmount" target="WCParam" value=""/>
...
<%-- Harish Ends : Added Loyalty related code snippet v8--%>

<%-- Harish Starts : Added Loyalty related code snippet of V7--%>
<%-- 
...
<c:set var="orderTotlaAmount" value="${order.orderAmount.grandTotal.value}" />
<c:set var="remainingBalanceAmount" value="" />
<c:set var="paidLoyaltyAmount" value="" />

<c:forEach var="paymentInstance" items="${order.orderPaymentInfo.paymentInstruction}" varStatus="paymentCount">
	<c:if test="${paymentInstance.paymentMethod.paymentMethodName == 'Loyalty'}">
		<c:set var="remainingBalanceAmount" value="${order.orderAmount.grandTotal.value - paymentInstance.amount.value}" />
		<c:set var="paidLoyaltyAmount" value="${paymentInstance.amount.value}" />
	</c:if>
</c:forEach>

<input type="hidden" value="${paidLoyaltyAmount}" name="loyaltyAmount" id="loyaltyAmountId" />
<c:set var="remainingAmount" value="${remainingBalanceAmount}" />
<c:set var="amount" value=" " />
<c:choose>
	<c:when test="${!empty remainingAmount }">
		<c:set var="orderTotlaAmount" value="${remainingAmount}" />
	...
	<c:otherwise>
		<input type="hidden" name="amount" value="<c:out value='${order.orderAmount.grandTotal.value}'/>" id="amountId" />
	</c:otherwise>
</c:choose>
...
<%-- Harish Ends : Added Loyalty related code snippet --%>
...
<%-- Harish Starts : Added this Loyalty related code snippet --%>
<wcf:url var="LoyaltyCalculationURL" value="LoyaltyOffline">
	<wcf:param name="langId" value="${WCParam.langId}" />
	<wcf:param name="storeId" value="${WCParam.storeId}" />
	<wcf:param name="catalogId" value="${WCParam.catalogId}" />
	<wcf:param name="orderId" value="${order.orderId}" />
</wcf:url>
<%-- Harish Ends : Added this Loyalty related code snippet --%>
									...
									<div id="mainContents" style="display:block">
										<div id="box">                                          
											...
											<flow:ifEnabled feature="SharedShippingBillingPage">
												...
												<%-- Harish Starts : Added Loyalty related code sinppet Using Custom DataBean --%>
												<wcbase:useBean id="userDB" classname="com.ibm.commerce.user.beans.UserDataBean">
													<c:set target="${userDB}" property="dataBeanKeyMemberId" value="${person.userId}" />
												</wcbase:useBean>	
												<c:set var="showLoyaltyPoints" value="true"/>
												<c:set var="remainingLoyaltyPointAmount" value=""/>
											    <input type="hidden" name="remainingAmount" value="<c:out value='${remainingAmount}'/>" id="remainingAmountId"/>
												<div class="" id="WC_LoyaltyDisplay_div">
														<c:if test='${!empty userDB.userField1 && userDB.userField1 ne 0 && userDB.userField1 ne "0" }'>
															<%-- <c:set var="showLoyaltyPoints" value="true"/> --%>
															<c:forEach var="paymentInstance" items="${paymentInstruction.paymentInstruction}" varStatus="paymentCount">
																<c:if test="${paymentInstance.payMethodId == 'Loyalty'}">
																<c:set var="remainingLoyaltyPointAmount" value="${userDB.userField1 - paymentInstance.piAmount}"/>
																	<c:set var="showLoyaltyPoints" value="false"/>
																	You have paid <fmt:formatNumber value="${paymentInstance.piAmount}" type="currency" maxFractionDigits="${env_currencyDecimal}" currencySymbol="${env_CurrencySymbolToFormat}"/>  
																	<c:out value="${CurrencySymbol}"/> with your loyalty points. Your new balance is : <fmt:formatNumber value="${userDB.userField1 - paymentInstance.piAmount}" type="number"/>
																</c:if>
															</c:forEach>
															<c:if test="${showLoyaltyPoints}">
															You Have <c:out value='${userDB.userField1}'/> Loyalty Points <br/>
															</c:if>
															<br/>
															<b> Click here to pay through Loyalty Points.</b>
															<input type="checkbox" name="" id="loyaltyCheckBoxStatus" value="loyaltyCheckBox" onclick="javascript:CheckoutPayments.showLoyaltyPayment();" />
														</c:if>
													<div class="" id="WC_LoyaltyPaymentTextDisplay_Div" style="display:none">
														<input type="text" name="loyaltyPointsText" value="" id="loyaltyPointsId" onkeypress="javascript:return numbersonly(event)" />
														<%-- <input type="button" name="" value="Pay" onclick="javascript:CheckoutPayments.calculatePayment('<c:out value='${LoyaltyCalculationURL}' />','<c:out value='${remainingLoyaltyPointAmount}'/>');"  /> --%>
														<input type="button" name="" value="Pay" onclick="javascript:CheckoutPayments.calculatePayment('<c:out value='${LoyaltyCalculationURL}' />','<c:choose><c:when test='${showLoyaltyPoints}'><c:out value='${userDB.userField1}'/></c:when><c:otherwise><c:out value='${remainingLoyaltyPointAmount}'/> </c:otherwise></c:choose>');"  />
													</div>
												</div>
												<%-- Harish Ends : Added Loyalty related code sinppet Using Custom DataBean --%>
												
												<%-- Harish Starts : Added Loyalty related code sinppet --%>
												<%--<c:set var="showLoyaltyPoints" value="true"/>
												<c:set var="remainingLoyaltyPointAmount" value=""/>
											    <input type="hidden" name="remainingAmount" value="<c:out value='${remainingAmount}'/>" id="remainingAmountId"/>
												<div class="" id="WC_LoyaltyDisplay_div">
														<c:if test='${!empty person.attributes["userField1"] && person.attributes["userField1"] ne 0 && person.attributes["userField1"] ne "0" }'>--%>
															<%-- <c:set var="showLoyaltyPoints" value="true"/> --%>
															<%--<c:forEach var="paymentInstance" items="${order.orderPaymentInfo.paymentInstruction}" varStatus="paymentCount">
																<c:if test="${paymentInstance.paymentMethod.paymentMethodName == 'Loyalty'}">
																<c:set var="remainingLoyaltyPointAmount" value="${person.attributes['userField1'] - paymentInstance.amount.value}"/>
																	<c:set var="showLoyaltyPoints" value="false"/>
																	You have paid <fmt:formatNumber value="${paymentInstance.amount.value}" type="currency" maxFractionDigits="${env_currencyDecimal}" currencySymbol="${env_CurrencySymbolToFormat}"/>  
																	<c:out value="${CurrencySymbol}"/> with your loyalty points. Your new balance is : <fmt:formatNumber value="${person.attributes['userField1'] - paymentInstance.amount.value}" type="number"/>
																</c:if>
															</c:forEach>
															<c:if test="${showLoyaltyPoints}">
															You Have <c:out value='${person.attributes["userField1"]}'/> Loyalty Points <br/>
															</c:if>
															<br/>
															<b> Click here to pay through Loyalty Points.</b>
															<input type="checkbox" name="" id="loyaltyCheckBoxStatus" value="loyaltyCheckBox" onclick="javascript:CheckoutPayments.showLoyaltyPayment();" />
													...
													<div class="" id="WC_LoyaltyPaymentTextDisplay_Div" style="display:none">
														<input type="text" name="loyaltyPointsText" value="" id="loyaltyPointsId" onkeypress="javascript:return numbersonly(event)" />--%>
														<%-- <input type="button" name="" value="Pay" onclick="javascript:CheckoutPayments.calculatePayment('<c:out value='${LoyaltyCalculationURL}' />','<c:out value='${remainingLoyaltyPointAmount}'/>');"  /> --%>
														<%--<input type="button" name="" value="Pay" onclick="javascript:CheckoutPayments.calculatePayment('<c:out value='${LoyaltyCalculationURL}' />','<c:choose><c:when test='${showLoyaltyPoints}'><c:out value='${person.attributes["userField1"]}'/></c:when><c:otherwise><c:out value='${remainingLoyaltyPointAmount}'/> </c:otherwise></c:choose>');"  />
													</div>
												</div>--%>
												<%-- Harish Ends : Added Loyalty related code sinppet --%>
